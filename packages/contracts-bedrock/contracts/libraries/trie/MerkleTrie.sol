        ...
        )
    {
        ...
        for (uint256 i = 0; i < _proof.length; i++) {
            ...
            if (currentNode.decoded.length == BRANCH_NODE_LENGTH) {
                ...
                bytes memory pathRemainder = BytesLib.slice(path, offset, path.length);
                bytes memory keyRemainder = BytesLib.slice(key, currentKeyIndex, key.length);
        ...
        return (pathLength, BytesLib.slice(key, currentKeyIndex, key.length), isFinalNode);
    }
